...
import './map.css';
import { getAllMarkers, getComments, addComment, updateMarkerStatus, deleteMarker, updateMarker, uploadPhoto } from './supabase.js';
...
export async function addMarkerToMap(map, markerData) {
    ...
    const marker = new maplibregl.Marker({ color: getMarkerColor(markerData.type, markerData.status) })
...
async function createPopupContent(markerData) {
    ...
    let actionsHtml = '';
    if (appState.currentUser && (appState.currentUser.id === user_id || appState.userProfile?.is_admin)) {
...
function getMarkerColor(type, status) {
    const baseColors = { 'Hive': '#ffaa00', 'Swarm': '#ff6600', 'Structure': '#666666', 'Tree': '#00aa00' };
    const statusModifiers = { 'Active': 1.0, 'Checked': 0.8, 'Unverified': 0.6, 'Gone': 0.3, 'Removed': 0.2 };
...
function getStatusEmoji(status) {
    const emojis = { 'Unverified': '⚪', 'Active': '🟢', 'Checked': '🟡', 'Gone': '🔴', 'Removed': '🗑️' };
